...
while time.time() < start + 5:
    ...
    assert resp.json() == {"http": True, "database": True, "cache": True, "email": True}
...
assert resp.json() == {"http": True, "database": False, "cache": True, "email": True}
...
resp = c.get("/health", timeout=5)
assert resp.status_code == 500
assert resp.json() == {"http": True, "database": True, "cache": False, "email": True}
...
resp = c.get("/health", timeout=5)
assert resp.status_code == 500
assert resp.json() == {"http": True, "database": True, "cache": True, "email": False}
